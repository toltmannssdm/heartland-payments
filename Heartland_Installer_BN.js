...
define([
	...
	function beforeInstall(context) {
        ...
        log.debug({title: 'heartlandPaymentMethodExists', details: heartlandPaymentMethodExists});
	...
	function afterInstall(context) {
		...
		var settingsExist = search.create({
			type: connection_settings.type,
			filters: [['isinactive', search.Operator.IS, false]]
		}).run().getRange({start: 0, end: 1});

        log.debug({title: 'settingsExist', details: settingsExist});
		...
		var settingsId = heartlandSettings.save();
        log.debug({title: 'settingsId', details: settingsId});

		var fieldId = (runtime.envType == 'PRODUCTION')
				? 'custscript_heartl_pmt_profile'
				: 'custscript_heartl_sandbox_pmt_profile';
